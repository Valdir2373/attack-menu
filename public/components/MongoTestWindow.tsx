...
interface MongoTestWindowProps {
  height: number;
  id: string;
  isFocused?: boolean;
  mongoUri?: string;
...
export const MongoTestWindow: React.FC<MongoTestWindowProps> = ({
  height,
  id,
  isFocused = false,
  mongoUri = "",
  inputFile = "github_results.txt",
  onBlur,
  onClose,
}) => {
  ...
  const [currentUri, setCurrentUri] = useWindowState(id, "currentUri", mongoUri);
  const [inputUri,   setInputUri]   = useWindowState<CursorInputField>(id, "inputUri", {
    value: mongoUri, pos: mongoUri.length,
  });


  const { mongoController } = useServices();

  const [massiveKeywords,  setMassiveKeywords]  = useWindowState<string[]>(id, "massiveKeywords",  DEFAULT_MONGO_KEYWORDS);
  ...
  const menuOptions = ["Validation Credentials", "Validation Massive", "Add MongoUri", "Sair"];
  ...
  const runValidation = async () => {
    ...
    const result = await mongoController.executeValidation(
      inputFile, FilesConfig.mongoCredentials,
      appendLog,
    );
  ...
  const runMassive = async (kws: string[], wl: string[], bl: string[]) => {
    ...
    const result = await mongoController.executeMassive(
      kws, FilesConfig.mongoCredentials,
      appendLog,
  ...
  useInput((input, key) => {
    ...
    if (mode === "menu") {
      ...
      if (key.return) {
        ...
        if (menuOptions[menuIndex] === "Validation Massive") {
          setMassiveKeywords([...DEFAULT_MONGO_KEYWORDS]);
          setMassiveWhitelist([]);
          setMassiveBlacklist([]);
          setKwField(EMPTY);
          setWlField(EMPTY);
        ...
        if (menuOptions[menuIndex] === "Add MongoUri") {
          setMode("input_uri");
          setInputUri({ value: currentUri, pos: currentUri.length });
          return;
        }
    ...
    if (mode === "input_uri") {
      if (key.return) {
        setCurrentUri(inputUri.value);
        setMode("menu");
        appendLog(`URI atualizada: ${inputUri.value}`);
      }
      return;
    }
    ...
    >
      ...
      <Box flexDirection="row" gap={1}>
        <Text color={isFocused ? FOCUS : UNFOCUS} bold>
          {isRunning ? <><Spinner /> MongoDB</> : "☘ MongoDB"}
        </Text>
      ...
      {mode === "massive_keywords" && (
        <Box flexDirection="column">
          ...
          {massiveKeywords.slice(-3).map((kw, i) => (
            <Text key={i} color={ACCENT}>{`  ${kw}`}</Text>
          ))}
      ...
      {mode === "massive_whitelist" && (
        <Box flexDirection="column">
          ...
          {massiveWhitelist.map((w, i) => (
            <Text key={i} color={ACCENT}>{`  ${w}`}</Text>
          ))}
      ...
      <Box flexDirection="column" flexGrow={1} overflow="hidden">
        {displayLogs.map((log, i) => (
          <Text key={i} wrap="truncate" color={PRIMARY}>
            {log}
          </Text>
        ))}
      </Box>
      ...
      <Box marginTop={0} flexDirection="column">
        <Text color={SECONDARY}>URI: {currentUri || "[vazio]"}</Text>
        <Text color={FOCUS}>
